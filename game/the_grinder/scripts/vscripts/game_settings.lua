...
--	local heroEntity = EntIndexToHScript()
	...
	local npc = EntIndexToHScript(unit_index)
	local hero = npc:GetPlayerOwner()
	local itemEntity = EntIndexToHScript(keys.ItemEntityIndex)
	local player = keys.PlayerID
	local itemname = keys.itemname

	if npc and not itemEntity.init then
		itemEntity.init = true
		local hItemToDrop = npc:TakeItem(itemEntity)
		CreateItemOnPositionForLaunch( npc:GetAbsOrigin(), hItemToDrop )
		hItemToDrop:LaunchLootInitialHeight( false, 0, 175, 0.3, npc:GetAbsOrigin() )
		npc:AddItem(itemEntity)
		UTIL_Remove( itemEntity:GetContainer())
--			npc:TakeItem(itemEntity)
--			npc:DropItemAtPositionImmediate(itemEntity, npc:GetAbsOrigin())
		Timers:CreateTimer(0, function()
--			npc:PickupDroppedItem(itemEntity:GetContainer())
		end)
	end
end
